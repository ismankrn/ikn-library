...
A long flat tail is wasted budget, and `Task(..., patience=N)` reclaims
it: the run stops once N consecutive iterations pass without improving
the best solution. It works with every algorithm in the library, since
the stop is decided by the task.

It is also the easiest way to lose a solution. The curve above is the
warning: its last improvement lands at iteration 12 of 15, so
`patience=5` would have stopped at iteration 9 and never seen it.
Choosing a value safely is a measurement, not a guess —
`task.stall_lengths()` provides the two numbers it rests on, and
[Early Stopping with `patience`](early-stopping.md) is the full
procedure, with one worked case where patience saves 60% of the budget
and one where it should not be used at all.
